  ...
  return <div className="app-shell">
    ...
    <main id="main-content" data-testid="simulation" data-time-ms={s.timeMs}>
      ...
      <details className="extra-options">
        <summary><span><strong>Lifecycle &amp; more</strong><small>Init, idle retirement, account quota &amp; speed</small></span><span className="disclosure-hint">{config.initEnabled || config.idleEnabled ? 'Lifecycle enabled' : 'Optional, off by default'}<ChevronDown size={18} aria-hidden="true" /></span></summary>
        <div className="options-content">
          <div className="lifecycle-options">
            ...
            <div><div className="option-heading"><Switch id="idle-switch" label="Illustrative idle timeout" checked={config.idleEnabled} onChange={idleEnabled => update({ idleEnabled })} tone="warm" /><label htmlFor="idle-switch">Illustrative idle timeout</label></div><NumberControl id="idle-number" label="Illustrative idle timeout in milliseconds" value={config.idleMs} min={1_000} max={3_600_000} step={1_000} suffix="ms" disabled={!config.idleEnabled} onChange={idleMs => update({ idleMs })} /><p>Warm environments appear as ochre diamonds, then retire. AWS does not guarantee warm retention times.</p></div>
          </div>
