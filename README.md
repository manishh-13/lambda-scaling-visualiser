...
- **Lifecycle & more:** directly above the simulation, with a visible summary of Init, idle retirement, account quota and speed. Init and idle retirement remain off by default. Open this section to enable them, pause/step, or try a scaling spike and short-duration high-volume traffic.
...
- With idle retirement off, reusable environments persist invisibly until reset. With it on, completed environments show a warm state and retire after the chosen illustrative timeout. AWS does not guarantee warm retention time.
